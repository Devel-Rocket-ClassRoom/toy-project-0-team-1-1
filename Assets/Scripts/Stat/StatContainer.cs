...
public class StatContainer
{
    ...
    private float ReCalculate() // 스탯 계산
    {
        ...
        var defaultValues = modifiers
            .Where(m => m.type == ModType.Percent && m.category == ModCategory.Default)
            .ToList();
        var mulValues = modifiers
            .Where(m => m.type == ModType.Percent && m.category == ModCategory.Multiply)
            .ToList();
